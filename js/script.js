...
addEventListener("keydown", function (e) {
	// Prevent page scrolling when space is pressed and track keys
...
addEventListener("keyup", function (e) {
	...
	f=0;
	// Prevent default on keyup for space too (keeps behaviour consistent)
...
var reset = function () {
	...
	bird.y=120;
	bird.score=0;
	// reset pipes to initial positions and scoring flags
	upper1.x = 20; upper1.y = -100; upper1.scored = false;
	upper2.x = 75; upper2.y = -50; upper2.scored = false;
	upper3.x = 130; upper3.y = -70; upper3.scored = false;
	lower1.x = 20; lower1.y = 150;
	lower2.x = 75; lower2.y = 135;
	lower3.x = 130; lower3.y = 160;
	// allow jump immediately after reset
	f = 0;
};
var f=0;
var difficulty=-40;
...
{
	// score is incremented when the bird passes a pipe (see below)
	// Allow jump with Up arrow (38) or Spacebar (32)
	if ((38 in keysDown || 32 in keysDown) && f==0) 
	{ // Player holding up
		bird.yspeed = -100;
		f=1;
	}
	...
	if (lower3.x<15 && bird.y>lower3.y-10)
		reset();
	// simple scoring: when a pipe passes the bird's x position, award 1 point
	// use a small threshold (25) matching the pipe width used elsewhere
	if (!upper1.scored && upper1.x + 25 < bird.x) { bird.score += 1; upper1.scored = true; }
	if (!upper2.scored && upper2.x + 25 < bird.x) { bird.score += 1; upper2.scored = true; }
	if (!upper3.scored && upper3.x + 25 < bird.x) { bird.score += 1; upper3.scored = true; }
};
